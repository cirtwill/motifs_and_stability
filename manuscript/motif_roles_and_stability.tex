...
\section{Introduction}
    ...
    \begin{figure}[hb!]
    [[Lots of comments on how this could be a very large box.]]
        \centering
        \includegraphics[width=.75\textwidth]{figures/motifs.eps}
        \caption{\textbf{A}) Food web structure can be described at several different scales. Global scale measures like species richness (S), link richness (L), and connectance (C = L/S$^2$) describe the network as a whole but do not give information on how the extinction risk differs for species within a food web (i.e., species A and B; labelled). Differences between these species are described by meso-scale and finer-scale descriptions. Three-species motifs provide a meso-scale description of species roles based on their direct and close indirect interactions. The role of species A is dominated by three-species chains (left-most motif) while the role of species B includes both many three-species chains and several apparent competition (second from right) and direct competition motifs. Fine-scale descriptions of species roles such as trophic level (height in the food web) and degree (number of interaction partners) [[not sure how much detail is necessary]]
        
        Finer-scale descriptors like trophic level (height within the food web) and degree (number of interaction partners) provide species-level information, but describe only a single dimension of network structure. Motif roles are a \emph{meso-scale} description of network structure and include species' direct and close indirect interactions in a detailed multi-dimensional framework. \\
        \textbf{B}) The small network in \textbf{A} as well as the small network inset in 
        The small network shown includes only four motifs: the three-species chain, omnivory, apparent competition, and direct competition motifs. These motifs all have clear ecological meanings. The three-species chain  has been important in the study of trophic cascades (e.g., sea otter, sea urchin, kelp). The omnivory motif describes a top predator and intermediate consumer who share a common prey and has been important in many models of food-web stability. The direct  
...
\section{Methods}
	...
	\subsection*{Testing effects of species' roles on time to extinction}
		...
		Relating motif roles to species degree}
