...
%DIF DEL Oikos_submission_2.tex          Mon May 30 14:59:56 2022
%DIF ADD motif_roles_and_stability.tex   Sun Jun 19 07:40:54 2022
\usepackage{amsmath} 
...
\section*{Abstract} % Up to 350 words for Ecology
    ...
    % Methods
    We test whether a species' time to extinction following a perturbation is related to its participation in stable and unstable motifs and assess how motif roles \DIFdelbegin \DIFdel{covary }\DIFdelend \DIFaddbegin \DIFadd{co-vary }\DIFaddend with a species' degree or trophic level.
    % Results
...
\section*{Introduction}
    ...
    \DIFaddbegin \DIFadd{`Stability' broadly refers to the tendency of a community to retain or return to a particular state, but it is a multifaceted concept incorporating resistance, resilience, asymptotic stability, variability, and robustness \mbox{%DIFAUXCMD
\citep{Donohue2013}}\hspace{0pt}%DIFAUXCMD
.
    `Disturbance' is similarly multifaceted, including short- or long-term and species-specific or community-wide disturbances, each with a different effect on a given metric of stability \mbox{%DIFAUXCMD
\citep{Donohue2013,Radchuck2019}}\hspace{0pt}%DIFAUXCMD
.
    ...
    }\DIFaddend Given these early indications that some motifs are more stable than others, we may also expect that a species' role-- here defined as the frequency with which it participates in different motifs --could affect its probability of extinction following a perturbation.
	Specifically, we expect that species participating more frequently in the stable motifs identified by~\citet{Borrelli2015a} are less likely to go extinct than species whose roles are dominated by other motifs.
	 \DIFdelbegin \DIFdel{While `stability' and `disturbance' are multifaceted concepts \mbox{%DIFAUXCMD
\citep{Donohue2013,Radchuck2019}}\hspace{0pt}%DIFAUXCMD
, we here focus on a dynamic measure of stability after a permanent disturbance --- the time to extinction after a primary extinction. 
	}\DIFdelend \DIFaddbegin 

	
	
	\DIFaddend Testing the hypothesis that participation in stable motifs will decrease a species' likelihood of extinction is complicated by the fact that species' motif roles are not independent of other aspects of fine-scale network structure. 
    In particular, a species' degree (number of predators and prey) is likely to strongly affect its motif role.
    The more interaction partners a species has, the more motifs \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend can participate in (Fig.~\ref{motifs}).
    \DIFdelbegin \DIFdel{This is likely to lead species }\DIFdelend \DIFaddbegin \DIFadd{Species }\DIFaddend with higher degrees \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{can therefore likely }\DIFaddend participate in a wider variety of motifs\DIFaddbegin \DIFadd{, }\DIFaddend as well as participate more frequently in the four common, stable motifs.
	...
	\subsection*{Statistical Analysis}
        ...
        \subsubsection*{Is motif participation related to persistence?}
            ...
            \textbf{Motif participation as a whole}
                ...
                To test whether motif participation as a whole is related to persistence, we fit a series of PERMANOVAs relating the Bray-Curtis dissimilarity in species' motif participation vectors to their persistence times \DIFaddbegin \emph{\DIFadd{per se}} \DIFadd{and Mantel tests relating dissimilarity in species' motif participation to dissimilarity in their persistence times }\DIFaddend (see \emph{Appendix S3} for details). 
                Due to computational constraints, we fit one PERMANOVA \DIFaddbegin \DIFadd{and one Mantel test }\DIFaddend per combination of network size and connectance (60 combinations). 
                We repeated these sets of PERMANOVAs \DIFaddbegin \DIFadd{and Mantel tests }\DIFaddend for each version of motif participation vectors (counts, species-normalised, and network-normalised; 180 \DIFdelbegin \DIFdel{PERMANOVAs }\DIFdelend \DIFaddbegin \DIFadd{of each test }\DIFaddend in total).
                \DIFaddbegin 

                
                \DIFaddend Because the assumption of equal varibility that underlies a PERMANOVA test was not met (see \emph{Appendix S4}), the results of these tests are indicative but not conclusive.
                ...
                \DIFaddbegin \DIFadd{The Mantel tests did not suffer from the same violation of assumptions but also mask which motif participation vectors are associated with high or low extinction risk; we therefore consider these tests likewise indicative but not conclusive (see }\emph{\DIFadd{Appendix S5}}\DIFadd{). We therefore present the results of these tests in the supplemental information only.
}\DIFaddend 
        ...
        \subsubsection*{Adding context: how does motif participation relate to simple roles?}
            ...
            To establish this context, we therefore fit a series of linear models (LMs) relating degree, trophic level, network size, and connectance to each version of the motif role (\emph{Appendix \DIFdelbegin \DIFdel{S6}\DIFdelend \DIFaddbegin \DIFadd{S7}\DIFaddend }). 
            ...
            To complete this background, we fit a regression of persistence against degree, trophic level, and their interaction, as well as a random effect of global network structure (\emph{Appendix \DIFdelbegin \DIFdel{S7}\DIFdelend \DIFaddbegin \DIFadd{S8}\DIFaddend }). 
...
\section*{Results}
    ...
    \subsection*{\DIFdelbegin \DIFdel{Motif participation was related to persistence}} \DIFdelend

        \DIFdelbegin \DIFdel{Our series of PERMANOVA tests demonstrated that species' overall motif roles (raw, species-normalised, and network-normalised) were correlated with their mean time to extinction across all combinations of species richness and connectance (Fig. \emph{S1}, Tables \emph{TS1-S3, Appendix S3}). Taken individually, each PERMANOVA was significant (all $p$\textless0.025). Moreover, after applying the correlated Bonferroni correction~\citep{Drezner2016}, all PERMANOVAs remained significant. However, some of these significant results are likely false positives since the variability of raw motif roles was not consistent across mean times to extinction for most combinations of species richness and connectance (Tables \emph{S4-6, Appendix S4}).  In most cases, motif roles were more variable among species with longer times to extinction (Fig. \emph{S3, Appendix S4}). This unequal variability means that the PERMANOVA results, while indicative, cannot be taken as conclusive proof that motif roles are related to species' time to extinction.}\DIFdelend

    \subsection*{Participation in stable motifs was related to persistence}

        When defining participation based on counts, increased participation in any stable motif except for omnivory was correlated with longer persistence following species removal (Fig. \emph{\DIFdelbegin \DIFdel{S9}\DIFdelend \DIFaddbegin \DIFadd{S10}\DIFaddend } and Table \emph{\DIFdelbegin \DIFdel{S12}\DIFdelend \DIFaddbegin \DIFadd{S13}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9}\DIFaddend }).
        Regressions against motif participation explained only 5.8\% of variation in persistence time (fixed effects only).
        When defining roles based on positions within each motif,  increased participation in any of the positions in the omnivory motif was correlated with shorter time to extinction while increased participation in any position in the apparent competition, direct competition, and three-species chain motifs was associated with longer persistence (Fig. \DIFdelbegin \DIFdel{~\ref{fig:persistence_positions}, Table }\DIFdelend \DIFaddbegin \emph{\DIFadd{S11}} \DIFadd{and  Table }\DIFaddend \emph{\DIFdelbegin \DIFdel{S14}\DIFdelend \DIFaddbegin \DIFadd{S15}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9}\DIFaddend }).
        Roles including positions explained the same amount of variation in persistence time as roles including whole motifs.

        
        When defining participation based on frequencies, greater participation in the omnivory, three-species chain, and apparent competition motifs was correlated with longer persistence, while the frequency of the direct competition motif was not significantly associated with persistence (Fig. \emph{\DIFdelbegin \DIFdel{S9}\DIFdelend \DIFaddbegin \DIFadd{S10}\DIFaddend } and, Table \emph{\DIFdelbegin \DIFdel{S16}\DIFdelend \DIFaddbegin \DIFadd{S17}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9}\DIFaddend }).
        ...
        Persistence time increased with the frequency of appearing in the bottom position in the apparent competition motif, the top position of the direct competition motif, and the middle position of the three-species chain motif (Fig.~\ref{fig:persistence_positions}, Table \emph{\DIFdelbegin \DIFdel{S18}\DIFdelend \DIFaddbegin \DIFadd{S19}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9}\DIFaddend }).

        
        When defining participation based on $Z$-scores, unusually high participation in the omnivory motif was associated with shorter persistence while unusually high participation in any other motif was associated with longer persistence (Fig. \emph{\DIFdelbegin \DIFdel{S9}\DIFdelend \DIFaddbegin \DIFadd{S10}\DIFaddend }, Table \emph{\DIFdelbegin \DIFdel{S20}\DIFdelend \DIFaddbegin \DIFadd{S21}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9}\DIFaddend }).
        ...
        Unusually high participation in the bottom position in the apparent competition motif, either position in the direct competition motif, and middle position in the three-species chain were associated with longer persistence (Fig. \DIFdelbegin \DIFdel{~\ref{fig:persistence_positions}, Table }\DIFdelend \DIFaddbegin \emph{\DIFadd{S12}} \DIFadd{and Table }\DIFaddend \emph{\DIFdelbegin \DIFdel{S22}\DIFdelend \DIFaddbegin \DIFadd{S23}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S8}\DIFdelend \DIFaddbegin \DIFadd{S9}\DIFaddend }).


    \subsection*{Motif participation was related to simpler roles}    

        As expected, a higher degree was associated with higher counts and higher $Z$-scores of any motif (Fig. \DIFdelbegin \emph{\DIFdel{S5}}%DIFAUXCMD
\DIFdel{, Appendix }\DIFdelend \emph{S6}\DIFaddbegin \DIFadd{, }\emph{\DIFadd{Appendix S7}}\DIFaddend ).
        A higher degree was also associated with higher counts of all positions and higher $Z$-scores of all positions except for the bottom and middle positions of the three-species chain (Fig. \emph{\DIFdelbegin \DIFdel{S6}\DIFdelend \DIFaddbegin \DIFadd{S7}\DIFaddend , Appendix \DIFdelbegin \DIFdel{S6}\DIFdelend \DIFaddbegin \DIFadd{S7}\DIFaddend }).
        ...
        The relationships between trophic level and participating in different motifs or positions were extremely weak for all versions of motif participation (\emph{Appendix \DIFdelbegin \DIFdel{S6}\DIFdelend \DIFaddbegin \DIFadd{S7}\DIFaddend }).
    ...
    \subsection*{Persistence was related to simpler roles}
        ...
        Species with higher degrees and lower trophic levels tended to have longer mean times to extinction (\emph{Appendix \DIFdelbegin \DIFdel{S7}\DIFdelend \DIFaddbegin \DIFadd{S8}\DIFaddend }).
...
\section*{Discussion}
    ...
    As expected, participation in positions in the apparent competition, direct competition, and three-species chain were especially associated with longer times to extinction.
 	...
 	\subsection*{Times to extinction are highly consistent}
		...
		The stronger correlation of \DIFdelbegin \DIFdel{extinction orders }\DIFdelend \DIFaddbegin \DIFadd{time to extinction }\DIFaddend in larger, more-connected networks, as well as the somewhat shorter mean times to extinction in these webs (Fig. S1, \emph{Appendix S1}) may be due to the greater number of short pathways by which an extinction somewhere in the web can affect a focal species. 
        ...
        \DIFaddbegin \DIFadd{These strong correlations of times to extinction may also be related to the extinction threshold we chose.
	...
	\DIFaddend \subsection*{Motif roles relate to extinction risk}
        ...
        In other cases, however, the association between the frequency of a position and persistence is opposite to what would be expected based on the position's association with degree, while trophic level was not strongly associated with motif roles (\emph{Appendix \DIFdelbegin \DIFdel{S6}\DIFdelend \DIFaddbegin \DIFadd{S7}\DIFaddend }).
        ...
        Earlier work has shown that the omnivory \DIFaddbegin \DIFadd{motif }\DIFaddend can be stable or unstable under different conditions and may or may not increase the overall stability of an entire food web~\citep{McCann1997,Emmerson2004,Borrelli2015a,Monteiro2016}.
        ...
        In our terms, \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{this suggests that when }\DIFaddend the top species in an omnivory motif is lost, the \DIFdelbegin \DIFdel{others will be strongly affected}\DIFdelend \DIFaddbegin \DIFadd{other species in the motif will tend to have substantially shorter times to extinction}\DIFaddend .
	...
	\subsection*{Moving forward with motifs}	
        ...
        \DIFaddbegin \DIFadd{Alternatively, future work exploring interactions between motifs and simple roles could improve our ability to predict extinction risk.
        ...
        \DIFadd{As previous work shows that traits of both the disturbed and responding species affect extinction risk~\mbox{%DIFAUXCMD
\citep{Wootton2016} }\hspace{0pt}%DIFAUXCMD
and that disturbance of basal resources can have particularly strong effects~\mbox{%DIFAUXCMD
\citep{Scherber2010}}\hspace{0pt}%DIFAUXCMD
, adding the trophic levels of both disturbed and focal species to future analyses may improve our predictions.
        ...
        }\DIFaddend While the interpretation of motif roles remains challenging, our results suggest that they are nevertheless a valuable tool and that the omnivory motif in particular may be useful when predicting responses to future disturbances.
    ...
    \subsection*{Speculation and alternative viewpoints}
        ...
        The choice of a lower threshold would permit species to remain extant at lower biomasses and lengthen times to extinction for the species most prone to reach low biomasses\DIFdelbegin \DIFdel{.
        ...
        A lower threshold would therefore likely require longer }\DIFdelend \DIFaddbegin \DIFadd{, therefore requiring longer and more computationally-expensive }\DIFaddend simulations  to detect differences in extinction risk between species.
        A lower extinction threshold might also reduce the impact of degree on persistence time as species would be more likely to retain all of their interaction partners\DIFdelbegin \DIFdel{.
        However, as interaction strengths depended upon the abundance of a partner, the effect of retaining a very rare predator prey would be minimal }\DIFdelend \DIFaddbegin \DIFadd{, although the influence of very rare prey is minimal when interaction strengths are weighted by abundance as we have done here}\DIFaddend .
...
\section*{Figure captions}
    ...
    \begin{figure}[ht!]
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.9\textwidth]{figures/motifs_box.eps}
%DIFDELCMD <         %%%
\DIFdelendFL %DIF >  \includegraphics[width=.9\textwidth]{figures/motifs_box.eps}
    ...
    \begin{figure}[ht!]
        \centering
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/roles/persistence_vs_positions_freq.eps}
%DIFDELCMD <         %%%
\DIFdelendFL %DIF >  \includegraphics[width=\textwidth]{figures/roles/persistence_vs_positions_freq.eps}
        \caption{Mean persistence time was significantly related to the frequency of appearing in different motif positions (\DIFaddbeginFL \DIFaddFL{marginal }\DIFaddendFL R$^2$=0.081\DIFaddbeginFL \DIFaddFL{, see also Table S19 }\emph{\DIFaddFL{Appendix S9}}\DIFaddendFL ). The figure is based on the fixed effects in a regression of persistence against participation in positions in the apparent competition, direct competition, omnivory, three-species chain and motifs, with all positions in the `other' (loop-containing; not shown) motifs grouped, as well as random effects of global network structure and network ID. Top positions have prey but no predators within the motif, bottom positions have predators but no prey within the motif, and middle positions have both prey and predators within the motif. We show the marginal effects of increasing frequencies of each position. As position frequencies must sum to 1, we weighted participation in other motifs by their mean values.}
        \label{fig:persistence_positions}
    \end{figure}

    \begin{figure}[ht!]
        \centering
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/positions_vs_Deg_freq.eps}
%DIFDELCMD <         %%%
\DIFdelendFL %DIF >  \includegraphics[width=\textwidth]{figures/positions_vs_Deg_freq.eps}
        \caption{\DIFaddbeginFL \DIFaddFL{Species-normalised (frequency) roles explained a great deal of variation in species' degrees (marginal R$^2$=0.729, see also Table S9, }\emph{\DIFaddFL{Appendix S7}}\DIFaddFL{). }\DIFaddendFL The frequencies of the bottom position in the direct competition motif and the bottom and middle positions in the three-species chain decreased with increasing degree while the frequencies of the other positions in our focal motifs increased. \DIFdelbeginFL \DIFdelFL{Species-normalised (frequency) roles explained 62.9\% of variation in degree. }\DIFdelendFL We show the marginal effects of increasing frequencies of each position. As position frequencies must sum to 1, we weighted participation in other motifs by their mean values.}
    ...
    \begin{figure}[ht!]
        \centering
        \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figures/roles/persistence_vs_degTL.eps}
%DIFDELCMD <         %%%
\DIFdelendFL %DIF >  \includegraphics[width=\textwidth]{figures/roles/persistence_vs_degTL.eps}
        \caption{\DIFaddbeginFL \DIFaddFL{Persistence time was strongly related to degree, trophic level, and their interaction (marginal R$^2$=0.211, see also Table S12, }\emph{\DIFaddFL{Appendix S8}}\DIFaddFL{). }\DIFaddendFL Mean persistence time increased with increasing degree for species at low trophic levels (STL 1-2) but decreased with increasing degree for species at high trophic levels (STL \textgreater2). The figure is based on the fixed effects in a regression of persistence against degree, trophic level, their interaction, and a random effect of global network structure. We also give the marginal R$^2$ for the regression (fixed effects only).}
