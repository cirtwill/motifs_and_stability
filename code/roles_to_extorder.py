...
def write_extorder_participation_file(particdict,extdict,outfile,s):
  f=open(outfile,'w')
  f.write('Species')
  for i in range(1,int(s)+1):
    f.write('\tRemoval_'+str(i))
  f.write('\t'+'\t'.join(['S5', 'S1', 'D3', 'S4', 'S2', 'D1', 'D4', 'D8', 'S3', 'D5', 'D2', 'D7', 'D6']))
  f.write('\n')
  for species in sorted(particdict):
    role=particdict[species]
    f.write('sp'+str(species))
    for removal in sorted(extdict):
      step=extdict[str(removal)]['sp'+str(species)].split('_')[1]
      f.write('\t'+step) 
  ...
  for s in sorted(os.listdir(motifdir)):
    ...
    # for c in ['0.06','0.08','0.1']:
      ...
      for j in range(0,100):
        ...
        write_extorder_participation_file(particdict,extdict,outfile2,s)
